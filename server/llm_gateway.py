...
async def call_llm(task_package: dict[str, Any]) -> dict[str, Any]:
    ...
    # ── Local Claude Engine branch ────────────────────────────
    if settings.local_claude_as_advisor_engine and settings.local_claude_worker_enabled:
        try:
            from server.services.local_claude_engine import call_local_claude
            import asyncio
            result = await asyncio.get_event_loop().run_in_executor(
                None, call_local_claude, task_package
            )
            # If Claude engine says fallback needed and fallback is configured, continue to mock/provider
            if result.get("engine_fallback_used") and settings.local_claude_engine_fallback_to_deepseek:
                # Fall through to mock/provider below
                pass
            elif result.get("engine_fallback_used"):
                # Fallback disabled — return Claude error as answer
                audit_id = log_model_call(
                    task_package["task_type"], task_package, [],
                    "local_claude_worker", result.get("warnings", []),
                    llm_mode="claude_worker_failed", provider="local_claude_worker",
                    used_openai=False, warnings=result.get("warnings", []),
                ...
                return {
                    **result, "audit_id": audit_id,
                    ...
                    "facts": [], "inferences": [],
                    "advisor_judgment": result.get("brief_answer", ""),
                    "action_suggestions": [], "risk_warnings": [],
                    "not_to_do": [], "uncertainty": "",
                    "llm_mode": "claude_worker_failed", "provider": "local_claude_worker",
                    "model": "claude-code", "raw_usage": {},
                }
            else:
                # Claude engine succeeded
                audit_id = log_model_call(
                    task_package["task_type"], task_package,
                    result.get("sources", []),
                    "local_claude_worker", result.get("warnings", []),
                    llm_mode="claude_worker", provider="local_claude_worker",
                    used_openai=False, warnings=result.get("warnings", []),
                    local_judge_status="passed",
                )
                return {
                    **result, "audit_id": audit_id,
                    "brief_answer": result.get("brief_answer", result.get("answer", "")),
                    "facts": result.get("sources", []), "inferences": [],
                    "advisor_judgment": result.get("answer", result.get("brief_answer", "")),
                    "action_suggestions": result.get("next_actions", []),
            ...
            if settings.local_claude_engine_fallback_to_deepseek:
                pass  # Fall through
            else:
                audit_id = log_model_call(
                    task_package["task_type"], task_package, [], "local_claude_worker",
                    [f"claude_engine_error:{exc.__class__.__name__}"],
                    llm_mode="claude_worker_error", provider="local_claude_worker",
                    used_openai=False, warnings=[f"claude_engine_error:{exc.__class__.__name__}"],
                    local_judge_status="passed",
                )
                error_msg = "本机外脑暂时没有接上，请确认 local_claude_worker.py 正在运行。"
                return {
                    "brief_answer": error_msg, "audit_id": audit_id,
                    "facts": [], "inferences": [], "advisor_judgment": error_msg,
                    "action_suggestions": [], "risk_warnings": [],
                    "not_to_do": [], "uncertainty": "",
                    "llm_mode": "claude_worker_error", "provider": "local_claude_worker",
                    "model": "claude-code", "raw_usage": {},
